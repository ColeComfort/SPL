#!/usr/bin/env bash
# Build docs/spl-run.pyz with embedded runner and vendored deps (lark).
set -euo pipefail
...
echo "[4/6] Vendor pure-Python deps into the zipapp (no network at runtime)"
# Lark provides pure wheels; zipapp can import them directly.
python3 -m pip install --upgrade --target "$DIST_STAGING" "lark>=1.1,<2"

echo "[5/6] Add embedded runner and minimal __main__"
...
echo "[6/6] Build zipapp and stage example"
mkdir -p "$OUT_DIR"
python3 -m zipapp "$DIST_STAGING" -o "$OUT_ZIP" -p "/usr/bin/env python3"
: > "$OUT_DIR/.nojekyll"

# Keep the example next to the web UI
