...
class FnDecl:
    name: str
    in_params: List[Tuple[str, str]]         # (name, "Dit"/"Qdit"/"Bool")
    out_types: List[str]                     # ["Dit"/"Qdit"] — empty for main
    kind: Optional[str]                      # Pauli|Clifford|Linear|Nonlinear; None for main
    body: List["Stmt"]
...
@dataclass
class QInitStmt(Stmt):
    name: str
    value: Optional[Union[int, Literal["mixed"]]]
...
class Apply(Stmt):
    gate: str
    args: List[str]
    outs: Optional[List[str]]

@dataclass
class QCtrlApply(Stmt):
    ctrl: str
    gate: str
    args: List[str]
    outs: Optional[List[str]]
...
@dataclass
class AssertRel(Stmt):
    relop: Literal["equal","included"]
    f1: str
    f2: str
...
vardecl: "let" IDENT ":" "Bool" ("=" bexpr)? ";"

init_stmt:  "init"  IDENT ("=" INT)? ";"
qinit_stmt: "qinit" IDENT ("=" (INT | "mixed"))? ";"

meas_stmt: "meas" IDENT ";"
prep_stmt: "prep" IDENT ";"

ret: "return" varlist? ";"
daggeras: "dagger" IDENT "as" IDENT ";"

// apply, qctrl, cctrl split into explicit-with/without-outs forms
apply_with_outs: "apply" IDENT "(" varlist? ")" "->" varlist ";"
apply_no_outs:   "apply" IDENT "(" varlist? ")" ";"

qctrl_with_outs: "qctrl" IDENT ":" "apply" IDENT "(" varlist? ")" "->" varlist ";"
qctrl_no_outs:   "qctrl" IDENT ":" "apply" IDENT "(" varlist? ")" ";"

cctrl_with_outs: "cctrl" IDENT ":" "apply" IDENT "(" varlist? ")" "->" varlist ";"
cctrl_no_outs:   "cctrl" IDENT ":" "apply" IDENT "(" varlist? ")" ";"

varlist: IDENT ("," IDENT)*

EQUAL: "equal"
INCLUDED: "included"
assertion: "assert" (EQUAL | INCLUDED) IDENT IDENT ";"

printspl: "print" "spl" IDENT ";"
...
?bor: band ("or" band)*
?band: bnot ("and" bnot)*
?bnot: "not" bnot -> not | cmp

?cmp: aexpr (COMP aexpr)?
COMP: "==" | "!=" | "<" | "<=" | ">" | ">="
...
PARSER = Lark(GRAMMAR, parser="lalr", start="start", transformer=_ToAST())

def parse_splpp(src: str) -> Program:
...
UNITARY_GATES_1 = {"X","Z","S","F","T"}
UNITARY_GATES_2 = {"CX","SWAP"}   # CZ synthesized via F;CX;F^3
PARAM_UNITARY = {"MUL"}           # e.g., MUL_3
# 'and' is classical multiplication primitive; treat as nonlinear in kind inference
CLASSICAL_ASSIGN = {"sum","copy","plusone","and"}
...
_KIND_LEQ = {
    "Pauli": {"Pauli"},
    "Clifford": {"Clifford","Pauli"},
    "Linear": {"Linear","Clifford","Pauli"},
    "Nonlinear": {"Nonlinear","Linear","Clifford","Pauli"},
}
    ...
    def _compile_block(self, env: Env, buf: SPLBuffer, body: List["Stmt"], fn_kind: Optional[str]):
        ...
        def ensure_kind(need: str, feature: str):
            ...
            if _KIND_POS[fn_kind] < _KIND_POS[need]:
                raise TypeError(f"quantum control over @{fn_kind} not supported") if feature.startswith("quantum control") else (_ for _ in ()).throw(TypeError(f"{feature} requires @{need}, found @{fn_kind}"))
